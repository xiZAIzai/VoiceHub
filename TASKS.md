...
### v0.5.0 发版（2026-08-31 深夜，方向一定案：先发布后移动端）

- [x] 发版前「点击即用」修正：**新装默认引擎 shandianshuo → builtin**
  （原默认对未装闪电说的新用户 = 听写静默无反应；现填 Key 即用，
  闪电说用户设置页一键切换，仪表盘下拉顺序同步）；官网「检测到闪电说在线
  → 优先」改为如实的「设置页指定谁就用谁」（该自动分流未实现，不虚假宣传）。
- [x] __version__ 0.5.0；官网徽标 v0.5.0 · 多厂商转写内核 + 三主题仪表盘。
- [x] tag v0.5.0 → CI 四产物 + 自动 Release。
- [ ] 下一里程碑（方向二）：移动端员工机（Termux/Android 接收端，粘贴权限是主要风险）。
