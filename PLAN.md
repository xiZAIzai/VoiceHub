...
> 最后更新：2026-08-20（V1 主体闭环：M0/M1/M3/M4/M5 完成、M2 笔记本通过；V2 桌面化立项）
> 当前焦点：V2 桌面化打包与开源分发（M6）；V1 尾巴：平板部署
...
- **V2 已立项（M6）**：桌面化打包与开源分发，路线定案见 ADR-6。
...
### ⏳ Milestone 6（V2）：桌面化打包与开源分发（未启动，见 ADR-6）

- PyInstaller 打包台式机 daemon 为 exe（托盘 + 图标 + 自动开窗，双击即用）。
...
5. V2 桌面化（M6）：一键启动 exe → 配置界面 → pywebview 窗口 → 开源工程化（ADR-6）。
