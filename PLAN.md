...
> 最后更新：2026-08-20（台式机↔笔记本全链路真机验证通过，M3 完成；67 单测全绿）
...
- **v1 代码已完成**：核心包 `voicehub/` 全部模块就绪，67 个单测全绿。
- **全链路真机验证通过（2026-08-20）**：台式机（Windows）Alt+2 → 闪电说转写 → 剪贴板拦截
  → UDP 发现笔记本 → HTTP 推送 → 笔记本自动粘贴，交互式验证 OK。
- 架构分层落地：入口 / 编排 / 状态机 / 剪贴板 / 发现 / 路由 / 存储 / 接收端 / 仪表盘各司其职。
- 待办：平板接收端部署；体验调优（热键换键决策、延迟压缩）。
