...
> 最后更新：2026-08-31 深夜（**v0.5.0 已发布**：多厂商转写内核 + 润色七厂商预设 +
> 三主题仪表盘 + vendor 本地化秒开 + 官网/使用文档；V3/V4 过程记录已归档）
> 当前焦点：**M14 移动端员工机**（Android/Termux 接收端，先真机验证粘贴路线再定案）；
> 官网 Pages 开关待用户开启；M12/M13 少量余项与 V2 用户侧验证随用随验
...
### ✅ V4：自建转写内核·云端 ASR（M11~M13 ✅；多厂商开放轮收口于 v0.5.0，2026-08-31）
...
#### Milestone 14：移动端员工机（2026-08-31 立项，方向定案）

> 把 Android 手机纳入「员工机」：一个人调度一支团队的第一块移动拼图。
> 现成资产：tablet_server.py（HTTP /paste + /health，v1 已实现未真机验证）。
> 关键风险：Android 10+ 限制后台模拟粘贴——Root 注入 / 无障碍服务 APK /
> 「剪贴板+通知一键粘贴」零权限降级，三条路线先真机验证再定案。
> 任务勾选进度在 TASKS.md「M14」。
...
1. ~~V4/M11 最小闭环竖切~~ ✅ 2026-08-26~27（含 M12-① 润色、悬浮框终态、
   wl-copy 双写、快速触发脚本——用户验收通过为日常主力流程）。
   剩余：仪表盘实时状态卡、引擎切换设置页 UI、防抖守卫（见 TASKS.md V4）。
